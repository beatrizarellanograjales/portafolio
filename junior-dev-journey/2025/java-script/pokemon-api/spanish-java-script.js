...
// Bucle que recorre los números del 1 al 151 (para obtener los primeros 151 Pokémon)
for (let i = 1; i <= 151; i++) {
    // Realiza una petición a la API de Pokémon concatenando el número actual de `i`
    fetch(URL + i)
        .then((response) => response.json()) // Convierte la respuesta a formato JSON
        .then(data => mostrarPokemon(data)) // Llama a la función `mostrarPokemon` pasando los datos del Pokémon obtenido
}
